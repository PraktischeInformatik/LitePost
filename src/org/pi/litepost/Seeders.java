...
class Posts implements IDatabaseSeeder {
	...
	@Override
	public void seed(DatabaseQueryManager db) throws Exception {
		db.executeQuery("insertPost", "What is love?", "Baby don't hurt me...", LocalDateTime.now(), "0090/99913213", 1);
		db.executeQuery("makeEvent", 1 , LocalDateTime.now());
		...
		db.executeQuery("insertPost", "Mein supertolles neues Fahrrad", "Mir wurde zu Weihnachten ein supertolles neues Fahrrad geschenkt, welches ich aber nicht brauche, also wollte ich es verkaufen.", LocalDateTime.of(2015, 3, 15, 5, 20), "Schlachthausstraße 19", 3);
		db.executeQuery("insertPost", "Gamer PC DDDDD FFFFF 00000 1 x 109191000hz TKraft 73", "", LocalDateTime.now(), "Burgumstraße 20", 4);
		db.executeQuery("insertPost", "Schärfe-Wettessen", "Beispiel-Text zur Demonstration des Forums.", LocalDateTime.now(), "I-wo beliebiges", 2);
	}
}
...
class Comments implements IDatabaseSeeder {
	...
	@Override
	public void seed(DatabaseQueryManager db) throws Exception {
		db.executeQuery("insertComment", 3, "lol", LocalDateTime.now(), 0, 1);
		db.executeQuery("insertComment", 1, ":D", LocalDateTime.now(), 1, 1);
	}
}
